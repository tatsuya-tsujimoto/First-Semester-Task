...
int CrateGraphHandle[Cratekind];		// クレートの種類最大数
...
void DrawObject( int x, int y, int type )
{
	...
	// 地面の描画
	DrawGraph(x, y, GroundGraphHandle, TRUE);
	// 配置物の描画
	DrawGraph(x, y, BlockGraphHandle, TRUE);
}
...
{
	DrawGraph(x,y, PlayerGraphHandle[0][0],TRUE);// ※※　描画処理をコーディング
	// まずは１つの画像を表示するだけを目指す
