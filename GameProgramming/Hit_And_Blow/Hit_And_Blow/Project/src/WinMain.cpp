...
// 定数定義
// ==============================
#define DIGITS 4//※※　DIGITSという名前の定数or定義で値は４	// 問題の桁数
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		if( gameClear==false )
		{
			...
			{
				num[cursor] += 1;//※※	// num配列のcursor番目を 1 増やす
				if (num[cursor] > 9)
					num[cursor] = 0;
			...
			{
				num[cursor] -= 1;//※※	// num配列のcursor番目を 1 減らす
				if (num[cursor] < 0)
					num[cursor] = 9;
